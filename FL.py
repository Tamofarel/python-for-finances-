...
def save_accounts(accounts, filename):
    """Write the accounts dict to filename as JSON."""
    with open(filename, "w") as file:
        json.dump(accounts, file, indent=2)


def create_account(accounts, name):
    """Add a new account with balance 0 and empty history."""
    if name in accounts:
...
def deposit(accounts, name, amount):
    """Add amount to the named account's balance, if valid."""
...
def withdraw(accounts, name, amount):
    """Subtract amount from the named account's balance, if valid and affordable."""
    if name not in accounts:
        print(f"Error: account '{name}' does not exist.")
        return
    if amount <= 0:
...
def transfer(accounts, from_name, to_name, amount):
    """Move amount from one account to another, only if it can fully succeed."""
...
def list_accounts(accounts):
    """Print every account name and balance."""
    if len(accounts) == 0:
...
def show_history(accounts, name):
    """Print the transaction history for one account."""
    if name not in accounts:
        print(f"Error: account '{name}' does not exist.")
        return
